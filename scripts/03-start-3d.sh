...
#    Модели (локальные, см. models/README.md; ~14GB вместе):
#      hunyuan3d-dit-v2-1 (shape, ~6.9GB) + hunyuan3d-paintpbr-v2-1 (PBR-текстуры)
#      + hunyuan3d-vae-v2-1 (~0.6GB)
#    Пиковая VRAM ~20–26GB — в 32GB входит с запасом; страховка: HY3D_LOW_VRAM=1
# =============================================================================
set -euo pipefail
...
[[ -d "vendor/Hunyuan3D-2.1" ]] || die "нет vendor/Hunyuan3D-2.1 — сначала запустите ./scripts/00-setup.sh"
...
log "Запуск Hunyuan3D-2.1: GPU=$GPU_3D порт=$HY3D_PORT shape=$HY3D_SHAPE_SUBFOLDER"
log "Флаги: ${EXTRA_FLAGS[*]:-<нет>}  Лог: $LOG_DIR/hunyuan3d.log"
log "Загрузка ~14GB моделей займёт 2–5 минут..."
(
  cd "$ROOT_DIR/vendor/Hunyuan3D-2.1"
