...
```
 ...
 │ Qwen3.8-27B    │  OpenAI API │    — он же шлёт запросы генерации вниз
 │ NVFP4 + vision │  :8000/v1   │
 │ + FP8 KV, GPU01│             │
 └────────────────┘             │
...
|---|---|---|---|---|
| 0, 1 (или 0,1,2,3 TP4) | 1Cat-vLLM | Qwen3.8-27B-NVFP4-DFlash2 (dense 27B, 256k) | ~24 GB (TP2) / ~18 GB (TP4) | 0.90 × 32 = 28.8 GB |
...
### 1Cat-vLLM + Qwen3.8-27B (GPU 0–1, порт 8000)

Модель: **Qwen3.8-27B-NVFP4-DFlash2** (dense 27B, NVFP4, 24 GB) — эталонный
бандл 1Cat-vLLM для sm70 (модель её release-gate). Для 2×V100:

- **NVFP4-веса** (~0.5 байта/параметр): ~12 GB/карту при TP2
  (~6 GB/карту при TP4) — большой запас под KV-кэш;
- **3.8-поколение** — свежее, заточено под долгогоризонтные агентные задачи
  (MCP-сценарий); нативный контекст 256k;
- **мультимодальная**: через OpenAI API (content с картинкой) агент может
  «видеть» скриншоты сцены/игрового окна: Unity MCP с vision-тулзом
  превращается в агента, который видит результат своих действий;
...
`VLLM_SM70_FLASH_ATTN_V100=1`, `CUDA_DEVICE_ORDER=PCI_BUS_ID` — ставятся
скриптом из `config.env`. Для FP8 KV + NVFP4-чекпоинт перед стартом
идемпотентно применяется эталонный патч 1Cat (unit-scale e5m2).

Альтернативы (включая лёгкий QUASAR NVFP4 19.2GB, «безотказный» Qwen3-14B fp16
и MoE-30B-A3B под максимальный контекст) — в `models/README.md`.
...
| Статья | На карту |
|---|---|
| Веса Qwen3.8-27B-NVFP4-DFlash2 (24 GB) / 2 | ~12 GB |
| KV-кэш на 131 072 токенов, **FP8 E5M2** (64 сл. × 2 × 2 KV-головы/карту × 256 × 1B) | ~8.4 GB |
| Активации, CUDA-графы, NCCL, оверхед | ~3–4 GB |
| **Итого** | **~24 GB** из 32 (лимит vLLM: 28.8 GB) |

В «продакшен-режиме» (TP4, 256k): веса ~6 GB/карту, KV 262k ~8.4 GB/карту
(1 KV-голова/карту) → ~18 GB/карту — с большим запасом.

FP8 KV-кэш (1Cat-vLLM) вдвое сжимает KV относительно fp16. Запас ~6–8 GB
на карту. 256k-контекст в 3.8-27B по умолчанию **не влезает** (KV 64 слоя ×
256 dim тяжёлый: 16.8 GB/карту) — для длинного контекста либо
`LLM_MAX_MODEL_LEN=163840`, либо модель с лёгким KV (30B-A3B).
При нехватке: `LLM_MAX_MODEL_LEN=32768`, `LLM_GPU_MEM_UTIL=0.85` или
`LLM_KV_CACHE_DTYPE=auto` (полный fp16 KV вместо FP8 — кэш чуть больше,
но без потери точности значений KV).
