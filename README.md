...
```
 ...
 │ Qwen3-14B fp16 │  OpenAI API │    — он же шлёт запросы генерации вниз
 │ TP2, Flash-V100│  :8000/v1   │
 │ + FP8 KV, GPU01│             │
 └────────────────┘             │
...
|---|---|---|---|---|
| 0, 1 | 1Cat-vLLM (TP=2) | Qwen3-14B-Instruct-2507, fp16 | ~20–22 GB на карту | 0.90 × 32 = 28.8 GB |
...
### 1Cat-vLLM + Qwen3-14B (GPU 0–1, порт 8000)
...
| `--max-model-len 65536` | 64k | длинный контекст под агентные MCP-сессии |
...
| Статья | На карту |
|---|---|
| Веса Qwen3-14B (~14.8B парам., fp16 ≈ 29.6 GB) / 2 | ~14.8 GB |
| KV-кэш на 65 536 токенов, **FP8 E5M2** (40 сл. × 2 × 4 KV-головы/карту × 128 × **1B**) | ~2.6 GB |
| Активации, CUDA-графы, NCCL, оверхед | ~3–4 GB |
| **Итого** | **~20–22 GB** из 32 (лимит vLLM: 28.8 GB) |
