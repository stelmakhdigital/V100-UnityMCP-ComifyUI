...
Модель: **Qwen3.8-27B-QUASAR-NVFP4** (dense 27B, QAT-квантование NVFP4,
19.2 GB) — это рекомендуемая модель самого 1Cat-vLLM для sm70 и модель
её release-gate. Для 2×V100:

- **веса всего ~9.6 GB/карту** при TP2 (NVFP4 ≈ 0.5 байта/параметр) —
  огромный запас под KV-кэш;
...
- **DFlash2 (опция)**: `LLM_DFLASH2=1` + draft-модель 3.6 GB — спекулятивное
  ...
  но всё равно быстрее обычного decode).

Альтернативы (включая «безотказный» Qwen3-14B fp16 и MoE-30B-A3B под
максимальный контекст) — в `models/README.md`.
...
| Статья | На карту |
|---|---|
| Веса Qwen3.8-27B-NVFP4 (19.2 GB) / 2 | ~9.6 GB |
